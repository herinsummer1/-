...
def zernike_detection(path):
    ...
    c_img = cv2.adaptiveThreshold(blur_img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 7, 4)  # 自适应阈值
    ZerImgM00 = cv2.filter2D(c_img, cv2.CV_64F, M00)
    ZerImgM11R = cv2.filter2D(c_img, cv2.CV_64F, M11R)  # 实部
    ZerImgM11I = cv2.filter2D(c_img, cv2.CV_64F, M11I)  # 虚部
    ZerImgM20 = cv2.filter2D(c_img, cv2.CV_64F, M20)
    ZerImgM31R = cv2.filter2D(c_img, cv2.CV_64F, M31R)  # 实部
    ZerImgM31I = cv2.filter2D(c_img, cv2.CV_64F, M31I)  # 虚部
    ...
    for idx in scatter_arr:
        ...
        h = (ZerImgM00[i][j] - k * np.pi / 2 + k * np.arcsin(l_method2) + k * l_method2 * (1 - l_method2 ** 2) ** 0.5)/ np.pi
        # / np.pi
        k_value = 20.0
        l_value = 2 ** 0.5 / g_N
